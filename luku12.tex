...
Dijkstran algoritmi on mukavaa toteuttaa
vieruslistoja käyttäen.
Koska kaarilla on painot, meidän kannattaa tallentaa ne olioina:

\begin{code}
ArrayList<Kaari>[] verkko = new ArrayList<>();
\end{code}

Tallennamme jokaisesta kaaresta kohdesolmun ja painon:
...
Määrittelemme seuraavat taulukot, jotka pitävät kirjaa
solmujen etäisyyk\-sistä sekä käsitellyistä solmuista:
...
Tärkeä seikka algoritmin tehokkuuden kannalta on,
että löydämme joka vaiheessa nopeasti seuraavaksi
käsiteltävän solmun.
Hyvä tietorakenne tähän on prioriteettijono,
joka pitää yllä solmujen etäisyysarvioita:
...
class Solmu implements Comparable<Solmu> {
    ...
    public int compareTo(Solmu s) {
        if (etaisyys != s.etaisyys) {
            return etaisyys-s.etaisyys;
        } else {
            return id-s.id;
        }
    }
