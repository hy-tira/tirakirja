...
kaikkiin verkon solmuihin ajassa $O(nm)$.
Tämän jälkeen vuoroon tulee Dijkstran algoritmi,
joka tekee saman tehokkaammin ajassa $O(n+m \log n)$
...
verkon solmujen välillä ajassa $O(n^3)$.
...
\end{tikzpicture}
\end{center}
\caption{Painotettu verkko, jossa lyhin polku solmusta $1$ solmuun $5$
on $1 \rightarrow 3 \rightarrow 4 \rightarrow 5$.}
\label{fig:verpai}
\end{figure}
...
Oletamme, että haluamme selvittää lyhimmät
polut lähtösolmusta $1$ alkaen.
Alustamme etäisyydet seuraavasti:
