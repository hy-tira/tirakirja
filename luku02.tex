...
Tavoitteemme on nyt arvioida askelten määrää suhteessa
syötteen kokoon $n$.
...
funktioita, joita kutsutaan parametrilla $n$:
...
Ensimmäisessä tapauksessa rekursiivisia kutsuja on $O(n)$
ja jokainen kutsu vie aikaa $O(1)$,
joten aikavaativuus on $O(n)$.
Toisessa tapauksessa rekursiivisia kutsuja on $O(2^n)$,
...
mutta jos bittinä on 1, voimme valita \emph{minkä tahansa}
